System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, PhysicsSystem2D, v2, TiledMap, RigidBody2D, ERigidBody2DType, BoxCollider2D, EPhysics2DDrawFlags, Collider2D, Contact2DType, _dec, _dec2, _class, _class2, _descriptor, _crd, ccclass, property, game;
          ...
          for (var i = 0; i < layerSize.width; i++) {
            for (var j = 0; j < layerSize.height; j++) {
              ...
              if (tiled.grid != 0) {
                ...
                body.type = ERigidBody2DType.Static; //body.group = 1;

                var _collider = tiled.node.addComponent(BoxCollider2D);

                _collider.offset = v2(tiledCellSize.width / 2, tiledCellSize.height / 2);
                _collider.size = tiledCellSize; //collider.group = 1;

                _collider.apply();

                tiled.node.getComponent(RigidBody2D).group = 1;
                tiled.node.getComponent(BoxCollider2D).group = 1;
              }
            }
          } // 注册单个碰撞体的回调函数
...
//# sourceMappingURL=2cd61b332946e0ad648a6a5a5f76363fa8d31fd5.js.map